...
from .losses import Keypoint3DLoss, Keypoint2DLoss, ParameterLoss, Keypoint3DFullNLLLoss_wo_align
from . import MANO
import numpy as np
import pickle
import os
...
class HAMER_FULL(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode, init_renderer: bool = True):
        ...
        self.keypoint_3d_loss = Keypoint3DFullNLLLoss_wo_align()
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        pred_uncertainty_params = self.uncertainty_head(conditioning_feats, train)
        ...
        return output, pred_uncertainty_params

    def compute_loss(self, batch: Dict, output: Dict, uncertainty: Dict, train: bool = True) -> torch.Tensor:
        """
        ...
        pred_keypoint_3d_precision_L_diag = uncertainty['keypoint_3d_precision_diag']
        # precision = torch.einsum('bij, bjk -> bik', pred_keypoint_3d_precision_L,
        #                          torch.transpose(pred_keypoint_3d_precision_L, -2, -1))
        # keypoint_3d_cov = torch.linalg.inv(precision.float())
        # uncertainty['keypoint_3d_cov'] = keypoint_3d_cov
        ...
        loss_keypoints_3d = self.keypoint_3d_loss(pred_keypoints_3d, pred_keypoint_3d_precision_L, gt_keypoints_3d, pelvis_id=0, pred_keypoint_3d_precision_L_diag=pred_keypoint_3d_precision_L_diag)
    ...
    def training_step(self, joint_batch: Dict, batch_idx: int) -> Dict:
        ...
        output, pred_uncertainty_params = self.forward_step(batch, train=True)
        pred_mano_params = output['pred_mano_params']
        if self.cfg.get('UPDATE_GT_SPIN', False):
            self.update_batch_gt_spin(batch, output)
        loss = self.compute_loss(batch, output, pred_uncertainty_params, train=True)
        if self.cfg.LOSS_WEIGHTS.ADVERSARIAL > 0:
    ...
    def validation_step(self, batch: Dict, batch_idx: int, dataloader_idx=0) -> Dict:
        ...
        """
        # batch_size = batch['img'].shape[0]
        output, pred_uncertainty_params = self.forward_step(batch, train=False)
        loss = self.compute_loss(batch, output, pred_uncertainty_params, train=False)
        output['loss'] = loss

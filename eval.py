...
from hamer_uncertainty.utils import Evaluator, recursive_to
...
def run_eval(model, model_cfg, dataset_cfg, device, args):
    ...
    if metrics is not None:
        ...
        results_csv = os.path.join(results_dir, 'eval_regression.csv')
        save_eval_result(results_csv, metrics_dict, model_cfg.ckpt_path, args.dataset, error=error, iters_done=i, exp_name=None)

    if preds is not None:
        results_dir = os.path.join(args.results_folder, model_cfg.exp_name)
        os.makedirs(results_dir, exist_ok=True)
        results_json = os.path.join(results_dir, '%s.json' % args.dataset.lower())

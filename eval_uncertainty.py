...
from hamer_uncertainty.configs import dataset_eval_config, get_config
from hamer_uncertainty.datasets import create_dataset
from hamer_uncertainty.utils import UncertaintyEvaluator, recursive_to
from tqdm import tqdm
from scipy import stats

from hamer_uncertainty.configs import CACHE_DIR_HAMER
from hamer_uncertainty.models import HAMER, download_models, load_hamer_uncertainty, DEFAULT_CHECKPOINT
...
def main():
    ...
    model_cfg = './hamer_uncertainty/configs_hydra/model_config.yaml'
    model_cfg = get_config(model_cfg, update_cachedir=True)

    model, model_cfg = load_hamer_uncertainty(model_cfg)
...
def run_eval(model, model_cfg, dataset_cfg, device, args):
    ...
    if args.dataset in ['FREIHAND-VAL', 'HO3D-VAL', 'HO3D-VAL-V3']:
        metrics = None
    ...
    dataloader = torch.utils.data.DataLoader(dataset, args.batch_size, shuffle=args.shuffle,
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            with torch.no_grad():
                ...
                if model_cfg.model_type == None or model_cfg.model_type == 'deterministic':
                    out = model(batch)
                elif 'diag' in model_cfg.model_type:
                    out, uncertainty = model(batch)
                elif 'full' in model_cfg.model_type:
                    out, uncertainty = model(batch)
                elif 'ours' in model_cfg.model_type:
                    out, uncertainty, sampled_out = model(batch)
                else:
                    print(f"There is no model_type: {model_cfg.model_type}")
            evaluator(out, uncertainty, batch, model_cfg.model_type)
        error = None
    ...
    results_dir = os.path.join(args.results_folder, model_cfg.exp_name)
    os.makedirs(results_dir, exist_ok=True)
    results_json = os.path.join(results_dir, '%s_uncertainty.json' % args.dataset.lower())
    save_preds_result(results_json, evaluator.uncertainty)
...
def save_preds_result(
    ...
    uncertainty_list = [x.tolist() for x in uncertainty_preds]
    ...
    print('Dumped %d uncertainty predictions to %s' % (len(uncertainty_list), pred_out_path))
